...
plt.plot(ti_2,Source*1e-6,linewidth=LW)
plt.plot(ti_2,-Sink*1e-6,color='tab:red',linewidth=LW)

plt.xlim(0,max(ti_2))

plt.grid()
plt.title('Sources and sinks of alkalinity',fontsize=FS)
plt.ylabel('Alk flux [Mmol/s]',fontsize=FS)
plt.xlabel('Time [yr]',fontsize=FS)
plt.xticks(fontsize=FS-3)
plt.yticks(fontsize=FS-3)
plt.legend(['Source','|Sink|'],loc="lower left",fontsize=FS-5,ncol=1)
