!----------------------------------------------------------------------
!----------------------------------------------------------------------
!   SCPM: Simple Carbon Project model (LGM configuration - AMOC as control parameter)
!----------------------------------------------------------------------
!----------------------------------------------------------------------
...
! State the parameters of the model (first general, than per box)
        ...
        RCP1=RCP_CO2*(a_RCP*C1+b_RCP)+(1-RCP_CO2)*RCP_base        ! Redfield C to P ratio
        RCP2=RCP_CO2*(a_RCP*C2+b_RCP)+(1-RCP_CO2)*RCP_base        ! Redfield C to P ratio
        RCP5=RCP_CO2*(a_RCP*C5+b_RCP)+(1-RCP_CO2)*RCP_base        ! Redfield C to P ratio
        RCP7=RCP_CO2*(a_RCP*C7+b_RCP)+(1-RCP_CO2)*RCP_base        ! Redfield C to P ratio
        ...
        IF (P3>0) then
           RCP3=RCP1!RCP_VAR*(0.9*1/(6.9/1000*P3*1000+6/1000)+0.1*RCPD)+(1-RCP_VAR)*RCP
        ELSE
           RCP3=RCP
        ENDIF
        ...
        IF (P4>0) then
           RCP4=(RCP1*S1+RCP2*S2+RCP7*S7)/S4!RCP_VAR*(0.9*1/(6.9/1000*P4*1000+6/1000)+0.1*RCPD)+(1-RCP_VAR)*RCP
        ELSE
           RCP4=RCP
        ENDIF

        IF (P6>0) then
           RCP6=RCP4 !RCP_VAR*(0.9*1/(6.9/1000*P6*1000+6/1000)+0.1*RCPD)+(1-RCP_VAR)*RCP
        ELSE
           RCP6=RCP
        ENDIF
       ...
       ! Piston velocity
        ...
        PV5=(PV-2)*PV_VAR*(((2116.8-136.25*T5+4.7353*T5**2-0.092307*T5**3+0.0007555*T5**4)/660)**-0.5)+(PV-2)*(1-PV_VAR)
       ...
       ! Def of RA_i only needed to start from trivial solution 
        ...
        ! State the ODEs
        F(1)=(-(air1*V1+air2*V2+air5*V5+air7*V7)/Vat)+Ant/Vat             ! CO2
...
! NF_LGM_v12 (initial conditions case L-0, AMOC = 15 Sv)
