!----------------------------------------------------------------------
!----------------------------------------------------------------------
!   SCPM: Simple Carbon Project model (LGM configuration - other control paramter)
!----------------------------------------------------------------------
!----------------------------------------------------------------------
...
!     ---------- ----
      ...
      DOUBLE PRECISION hallo1,CC2,hallo2,T5_TEMP,Tadjust5
...
! State the parameters of the model (first general, than per box)
        ...
        CO2baseHol=145.53186722*(1d-6) ! CO2 concentration Holocene
        gamma1=31*(1d6)  ! Upwelling box 4 and 6
        gamma2=40*(1d6)  ! Upwelling box 1 and 3
        psi1=18*(1d6)    ! GOC
        psi2_base=15*(1d6)    ! AMOC base
	   ...
	   Tadjust5=T5_TEMP*0.54*5.35*log((CO2)/CO2baseHol)
        ELSE
           psi2=psi2_base
           Tadjust=0
	   Tadjust5=0
        ...
        ! Carbon
        TC=CO2*Vat+C2*V2+C1*V1+C3*V3+C5*V5+C6*V6+C7*V7   ! Sum of carbon without eliminated variable
        TC1=ComC*(1d22)*par(1) !  Total carbon in system (PgC)
        ...
        ENDIF
        ! Phosphate
        TP=P2*V2+P1*V1+P3*V3+P5*V5+P6*V6+P7*V7      ! Sum of phosphate without eliminated variable
        TP1=2.8733503182616036*(1d15)*par(1)            ! Total phosphate in system
        P4=(1/V4)*(TP1-TP)                              ! Eliminated variable
   
        !Alkalinity
        TA=Alk2*V2+Alk1*V1+Alk3*V3+Alk5*V5+Alk6*V6+Alk7*V7 ! Sum of alkalinity without eliminated variable
        TA1=3446475.31695093*(1d12)-(3358734.46282621*(1d12)-ComC*(1d22))*2*par(1)   ! Total alkalinity in system
       ...
       ! Piston velocity
        PV1=10**par(18)*(PV*PV_VAR*(((2116.8-136.25*T1+4.7353*T1**2-0.092307*T1**3+0.0007555*T1**4)/660)**-0.5)+PV*(1-PV_VAR))
        PV2=10**par(18)*(PV*PV_VAR*(((2116.8-136.25*T2+4.7353*T2**2-0.092307*T2**3+0.0007555*T2**4)/660)**-0.5)+PV*(1-PV_VAR))
        PV5=10**par(18)*((1)*PV_VAR*(((2116.8-136.25*T5+4.7353*T5**2-0.092307*T5**3+0.0007555*T5**4)/660)**-0.5)+(1)*(1-PV_VAR))
        PV7=10**par(18)*(PV*PV_VAR*(((2116.8-136.25*T7+4.7353*T7**2-0.092307*T7**3+0.0007555*T7**4)/660)**-0.5)+PV*(1-PV_VAR))
       ...
       ! Def of RA_i only needed to start from trivial solution 
        ...
        FCA7=FCA_TEMP*FCA7*(T7+2)/7+(1-FCA_TEMP)*FCA7
        ...
        ! New biology
        Z1=10**par(20)*((1-BIO)*Zo1+BIO*(gamma2*P3*yr*1+Pin*yr)*eps1*RCP1*1/S1)
        Z2=10**par(20)*((1-BIO)*Zo2+BIO*psi2*P3*yr*eps2*RCP2*1/S2)
        Z5=10**par(20)*((1-BIO)*Zo5+BIO*fract*psi1*P7*yr*eps5*RCP5*1/S5)
        Z7=10**par(20)*((1-BIO)*Zo7+BIO*(psi2+fract*psi1)*P4*yr*eps7*RCP7*1/S7)
...
! Parameters
      ...
      PAR(16) = 0
      PAR(17) = 0
      PAR(18) = 0   ! Control parameter for piston velocity
      PAR(19) = 0   ! Control parameter for rain ratio
      PAR(20) = 0   ! Control parameter for biological production
  
      
! Special solution from transient code 
! NF_LGM_v12: intial conditions case LC-0
       U(1)=145.53186722/(1d6)

       U(2)=1998741.98062906/(1d6)
       U(3)=2382754.22021904/(1d6)
       U(4)=177.36951999/(1d6)
       
       U(5)=2083282.0873691/(1d6)
       U(6)=2349785.77960434/(1d6)
       U(7)=267.47182019/(1d6)
       
       U(8)=2196804.65814556/(1d6)
       U(9)=2396216.29231859/(1d6)
       U(10)=1559.51374577/(1d6)
       
       !U(8)=2432255.96108528/(1d6)
       !U(9)=2457802.94206664/(1d6)
       !U(10)=2328.800678/(1d6)
       
       U(11)=2265178.54773214/(1d6)
       U(12)=2434380.07425796/(1d6)
       U(13)=1813.7845241/(1d6)
       
       U(14)=2427117.5449512/(1d6)
       U(15)=2484663.48162164/(1d6)
       U(16)=2122.40291909/(1d6)
       
       U(17)=2217267.16301304/(1d6)
       U(18)=2423426.83700107/(1d6)
       U(19)=1373.22800387/(1d6)

       U(20)=3358734.46282621/(1d10)
       !TA=3446475.31695093
